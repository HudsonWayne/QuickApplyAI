      ...
      <ul className="max-w-4xl mx-auto grid gap-8 sm:grid-cols-2">
        {jobs.map((job, index) => (
          <li key={index} className="bg-white border border-green-200 rounded-2xl shadow-lg p-6 hover:shadow-2xl transition-shadow duration-300">
            ...
            <a href={job.link} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 px-5 py-2 bg-green-600 text-white rounded-full font-semibold hover:bg-green-700 transition-colors duration-300 shadow">View Job &rarr;</a>
          </li>
        ))}
      </ul>
    </div>
